...
const INITIAL_STAFF = [
  ...
  {id:'M001',name:'Chukwudi Eze',dept:'Merchandisers',shift:'MOR',role:'staff',active:true,pin:'1234'},
  {id:'M002',name:'Ngozi Obi',dept:'Merchandisers',shift:'AFT',role:'staff',active:true,pin:'1234'},
...
const deptColor = dept => {
  const map = { Cashier:[T.yellowLight,'#B87E00'], Groceries:['#E8F5E9','#1D6F42'], Drinks:['#E3F2FD','#1565C0'], Toiletries:['#FCE4EC','#880E4F'], Household:['#F3E5F5','#6A1B9A'], Electronics:['#FFF8E1','#F57F17'], Backstore:['#EFEBE9','#4E342E'], Merchandisers:['#E0F2F1','#00695C'], Management:['#E8EEFA','#1A3C6E'] }
...
function generateMonthRoster(staff, year, month) {
  ...
  const activeStaff = staff.filter(s => s.active && s.id !== 'ADMIN')
  ...
  DEPARTMENTS.forEach(dept => {
    ['MOR', 'AFT'].forEach(shift => {
      const deptStaff = activeStaff.filter(s => s.dept === dept && s.shift === shift)
      if (deptStaff.length < 2) return
      for (let d = 1; d <= days; d++) {
        const dt = new Date(year, month, d)
        if (dt.getDay() === 0) continue
        const date = isoDate(dt)
        const present = deptStaff.filter(s => roster[s.id] && roster[s.id][date] !== 'OFF')
        if (present.length === 1) roster[present[0].id][date] = 'LNG'
      }
    })
  })
...
const inputStyle = { width:'100%', padding:'11px 14px', borderRadius:10, border:`1.5px solid ${T.border}`, fontSize:14, fontFamily:"'Nunito',sans-serif", outline:'none', background:T.white, color:T.dark, marginBottom:12, display:'block' }
...
// ─── DASHBOARD ────────────────────────────────────────────────────────────────
function Dashboard({ currentUser, staff, roster, assignments }) {
...
function RosterView({ currentUser, staff, roster, rosterStatus, monthKey, onMonthChange }) {
  ...
  const filtered = staff.filter(s=>s.active&&s.id!=='ADMIN'&&(deptFilter==='All'||s.dept===deptFilter)&&(canViewAll||s.dept===currentUser.dept||s.id===currentUser.id))
...
function Directory({ currentUser, staff, onUpdateStaff, onAddStaff, onDeactivate }) {
  ...
  const save = () => {
    ...
    else { const nid=form.id.trim().toUpperCase()||'S'+String(Date.now()).slice(-4); onAddStaff({...form,id:nid,active:true,pin:'1234'}) }
    setShowForm(false)
  }
  ...
  return (
    <div style={{padding:'12px 14px 80px'}}>
      ...
      <div style={{display:'flex',gap:5,overflowX:'auto',marginBottom:12,paddingBottom:4}}>
        {['All',...DEPARTMENTS].map(d=><button key={d} onClick={()=>setDeptF(d)} style={{...btnSm(deptF===d?'primary':'ghost'),flexShrink:0,fontSize:11}}>{d}</button>)}
      </div>
      ...
      <Modal show={showForm} onClose={()=>setShowForm(false)} title={editTarget?'Edit Staff':'Add Staff'}>
        ...
        <select value={form.dept} onChange={e=>setForm(f=>({...f,dept:e.target.value}))} style={selectStyle}>{DEPARTMENTS.map(d=><option key={d}>{d}</option>)}</select>
...
// ─── TASKS ────────────────────────────────────────────────────────────────────
function Tasks({ currentUser, staff, assignments, onUpdateAssignments }) {
    ...
    <div style={{padding:'12px 14px 80px'}}>
      ...
      <Modal show={showCreate} onClose={()=>setShowCreate(false)} title="New Assignment">
        ...
        <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:12}}>
          {['All',...DEPARTMENTS].map(d=><button key={d} onClick={()=>setForm(f=>({...f,departments:f.departments.includes(d)?f.departments.filter(x=>x!==d):[...f.departments,d]}))} style={{...btnSm(form.departments.includes(d)?'primary':'ghost'),fontSize:11}}>{d}</button>)}
        </div>
...
// ─── ADMIN ────────────────────────────────────────────────────────────────────
function Admin({ staff, roster, monthKey, onRegenerate, onMonthChange }) {
  const [year,month] = monthKey.split('-').map(Number)
  const days = monthDays(year,month)

  const byDept = useMemo(()=>{
    const r={}; DEPARTMENTS.forEach(d=>{ r[d]=staff.filter(s=>s.active&&s.dept===d&&s.id!=='ADMIN') }); return r
  },[staff])
    ...
    DEPARTMENTS.forEach(dept=>{
      ...
      ;['MOR','AFT'].forEach(shift=>{
        ...
        for(let d=1;d<=days;d++){
          ...
          if(present<Math.ceil(ss.length*0.5)) issues.push({dept,shift,date,present,total:ss.length})
        }
      })
    })
    return issues.slice(0,8)
  },[staff,roster,year,month,days])
  ...
  return (
    <div style={{padding:'12px 14px 80px'}}>
      ...
      <div style={card}>
        <div style={cardTitle}>Department Breakdown</div>
        {DEPARTMENTS.map(d=>(
          <div key={d} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'7px 0',borderBottom:`1px solid ${T.border}`}}>
            <span style={deptBadge(d)}>{d}</span>
            <div style={{display:'flex',gap:10}}>
              <span style={{fontSize:12,fontWeight:700,color:T.dark}}>{byDept[d].length} staff</span>
              <span style={{fontSize:11,color:T.gray}}>{byDept[d].filter(s=>s.shift==='MOR').length}M / {byDept[d].filter(s=>s.shift==='AFT').length}A</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={card}>
        ...
        <p style={{fontSize:12,color:T.gray,marginBottom:12}}>Regenerate the auto-roster for this month using the current staff database and shift rules.</p>
    ...
    <div style={{padding:'12px 14px 80px'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}>
        <div style={{...card,marginBottom:0,textAlign:'center',padding:14}}>
          ...
          <div style={{fontSize:10,color:T.gray,fontWeight:600,marginTop:3}}>Fixed Shift</div>
        </div>
        <div style={{...card,marginBottom:0,textAlign:'center',padding:14}}>
...
export default function App() {
  ...
  useEffect(()=>{ if(!Object.keys(roster).length){ const[y,m]=monthKey.split('-').map(Number); setRoster(generateMonthRoster(staff,y,m)) } },[])
  ...
  const changeMonth = dir => {
    ...
    setMonthKey(nk)
    try{ const s=localStorage.getItem('jr_r_'+nk); if(s){setRoster(JSON.parse(s))}else{setRoster(generateMonthRoster(staff,nd.getFullYear(),nd.getMonth()))} }catch{}
  }
  ...
  const regen = () => { const[y,m]=monthKey.split('-').map(Number); setRoster(generateMonthRoster(staff,y,m)); setRosterStatus('draft') }
  ...
  const renderPage = () => {
    switch(tab){
      case 'home': return <Dashboard currentUser={currentUser} staff={staff} roster={roster} assignments={assignments}/>
      case 'roster': return <RosterView currentUser={currentUser} staff={staff} roster={roster} rosterStatus={rosterStatus} monthKey={monthKey} onMonthChange={changeMonth}/>
      case 'directory': return <Directory currentUser={currentUser} staff={staff} onUpdateStaff={updateStaff} onAddStaff={addStaff} onDeactivate={deactivate}/>
      case 'tasks': return <Tasks currentUser={currentUser} staff={staff} assignments={assignments} onUpdateAssignments={setAssignments}/>
      case 'admin': return <Admin staff={staff} roster={roster} monthKey={monthKey} onRegenerate={regen} onMonthChange={changeMonth}/>
